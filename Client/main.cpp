...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	if(fdwReason == DLL_PROCESS_ATTACH)
	{
		...
		{
			MessageBox(0,"You are missing the vcpfnt.png/dat file from your games directory.","WellRo Launcher Error",MB_OK);
			SetForegroundWindow(HWND_DESKTOP);
			ExitProcess(1);			
		}
...
void InitD3DStuff()
{
	...
	pVersion->setText("WellRo Launcher - Development Version (" __TIME__ ", " __DATE__ ")");
	CEGUI::Font * pFont = pGUI->GetTahomaBoldFont();
	float fTextWidth = pFont->getTextExtent("WellRo Launcher - Development Version (" __TIME__ ", " __DATE__ ")");
